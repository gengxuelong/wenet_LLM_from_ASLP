...
def url_opener_mix_task(data):
    ...
    for sample in data:
        assert 'src' in sample
        # TODO(Binbin Zhang): support HTTP
        url, task_type = sample['src'].split()
        ...
        try:
            ...
            # network file, such as HTTP(HDFS/OSS/S3)/HTTPS/SCP
                ...
                stream = process.stdout
            sample.update(stream=stream)
            sample.update(task_type=task_type)
            yield sample
        except Exception as ex:
            logging.warning('Failed to open {}'.format(url))


def tar_file_and_group_mix_task(data):
    """ Expand a stream of open tar files into a stream of tar file contents.
        And groups the file with same prefix
...
def filter(data,
    ...
    """ Filter sample according to feature and label length
        ...
        Args::
            data_list: Iterable[{key, wav, label, sample_rate}]
...
def speed_perturb(data, speeds=None):
    """ Apply speed perturb to the data_list.
        Inplace operation.
    ...
    for sample in data:
        ...
        yield sample
def compute_raw_wav(data):
    """
    made by gengxuelong , 转换音频字典的key值,适配后续处理函数
    Args:
        data:
...
def compute_raw_wav_task_mix(data):
    """
    made by gengxuelong , 转换音频字典的key值,适配后续处理函数
    Args:
        data:
    ...
    for sample in data:
        ...
        if len(waveform.shape) != 1:
            waveform = waveform[0]
        yield dict(key=sample['key'], label=sample['label'], feat=waveform,task_type=sample['task_type'])
...
def sort(data, sort_size=500):
    """ Sort the data_list by feature length.
...
def static_batch(data, batch_size=16):
    """ Static batch the data_list by `batch_size`
...
def dynamic_batch(data, max_frames_in_batch=12000):
    """ Dynamic batch the data_list until the total frames in batch
...
def padding(data):
    """ Padding the data_list into training data_list
...
def padding_mix_task(data):
    ...
    for sample in data:
        ...
        sorted_feats = [sample[i]['feat'] for i in order]
        sorted_keys = [sample[i]['key'] for i in order]
        task_control_list = [sample[i]['task_type'] for i in order]
        sorted_labels = [
            torch.tensor(sample[i]['label'], dtype=torch.int64) for i in order
        ]
        label_lengths = torch.tensor([x.size(0) for x in sorted_labels],
        ...
        padding_labels = pad_sequence(sorted_labels,
                                      batch_first=True,
                                      padding_value=-100)
        if len(set(task_control_list)) != 1:
            continue
        yield {
            "keys": sorted_keys,
            "feats": padded_feats,
            ...
            "feats_lengths": feats_lengths,
            "target_lengths": label_lengths,
            "task_control_list": task_control_list
        }
