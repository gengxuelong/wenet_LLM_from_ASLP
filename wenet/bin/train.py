...
def main():
    ...
    It initializes various components such as tokenizer, distributed training, dataset, model, optimizer, scheduler, and executor.
    It then starts the training loop, logging the training and cross-validation information for each epoch.
    Finally, it saves the final model and closes the Tensorboard writer.
    """
    args = get_args()
    ...
    if configs.get('init_step_epoch', False):
        executor.step = 0
    ...
    if configs.get("init_step_epoch", False):
        start_epoch = 0
    if_break = (configs.get("dataset_conf",{})).get("break", False)
    if not if_break:
        configs.pop("init_infos", None) # 不想让他回复batch_size的连续,就不注释它
    final_epoch = configs.get('max_epoch', 100)
    for epoch in range(start_epoch, final_epoch):
        ...
        total_loss, num_seen_utts = executor.cv(model, cv_data_loader, configs,scaler)
