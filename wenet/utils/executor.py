...
class Executor:
    ...
    def train(self, model, optimizer, scheduler, train_data_loader,
        ...
        ''' Train one epoch
        '''
        if_jump_some_batch = False
        init_batch_idx = 0
        if configs.get('init_infos', None) is not None:
            tag_name = configs["init_infos"].get("tag", "init")
            init_epoch = configs["init_infos"].get("epoch", 0)
            now_epoch = configs['epoch']
            if_jump_some_batch = ('step_' in tag_name) and (init_epoch == now_epoch)
            logging.info('ASLP-LOG: if_jump_some_batch: {}'.format(if_jump_some_batch))
            init_batch_idx = configs["init_infos"].get("batch_idx", 0)
            logging.info('init_batch_idx: {}'.format(init_batch_idx))
        ...
        with model_context():
            for batch_idx, batch_dict in enumerate(train_data_loader):
                if if_jump_some_batch:
                    batch_idx = batch_idx + init_batch_idx
                info_dict["tag"] = "TRAIN"
                ...
                if self.step % save_interval == 0 and self.step != 0 \
                    ...
                    logging.info('训练中间保存模型')
                    total_loss, num_seen_utts = self.cv(
                        model, cv_data_loader, configs, scaler)
                    model.train()
                    info_dict.update({
                        "tag":
    ...
    def cv(self, model, cv_data_loader, configs, scaler):
        ''' Cross validation on
        '''
        model.eval()
        info_dict = copy.deepcopy(configs)
