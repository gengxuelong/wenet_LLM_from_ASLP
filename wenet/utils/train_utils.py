...
def add_dataset_args(parser):
    ...
    parser.add_argument('--train_data', required=True, help='train data_list file')
    parser.add_argument('--cv_data', required=True, help='cv data_list file')
...
def check_modify_and_save_config(args, configs, symbol_table):
    if args.train_engine == "torch_ddp":
        ...
        # NOTE(xcsong): DeepSpeed does not support uneven data_list. When using custom
        #   dataset, we need to manually ensure that the data_list is evenly distributed
        ...
        #   dataloader uses PyTorch's torch.utils.data_list.DistributedSampler which does
        #   not support IterableDataset, IterableDataset is extremly useful in large
        #   scale training because it lets you stream the data_list without having to
