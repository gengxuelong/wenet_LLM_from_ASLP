...
stage=0 # start from 0 if you need to start from data_list preparation
stop_stage=0

nj=16
dict=data_list/dict/lang_char.txt
...
set=test
ali_format=ali_format.data_list
ali_result=ali.res
blank_thres=0.9999
thres=0.00001
...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    # Prepare required data_list for ctc alignment
    echo "Prepare data, prepare required format"
    for x in $set; do
        tools/make_raw_list.py data_list/$x/wav.scp data_list/$x/text \
          ali_format
    done
fi
