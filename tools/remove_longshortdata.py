...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='remove too long or too short data_list in format.data_list')
    parser.add_argument('--data_file', type=str, help='input format data_list')
    parser.add_argument('--output_data_file',
                        type=str,
                        help='output format data_list')
    parser.add_argument(
        '--min_input_len',
        type=float,
        default=0,
        help='minimum input seq length, in seconds for raw wav, \
                            in frame numbers for feature data_list')
    parser.add_argument(
        '--max_input_len',
        type=float,
        default=20,
