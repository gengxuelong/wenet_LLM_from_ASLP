...
segments=$2
outscp=$3
data_list=$(dirname ${inscp})
if [ $# -eq 4 ]; then
  logdir=$4
else
  logdir=${data_list}/log
fi
mkdir -p ${logdir}
...
for slice in `ls $logdir/wav_*.slice`; do
{
    ...
    mkdir -p ${data_list}/wavs/${name}
    cat ${slice} | awk -F ',' -v sox=$sox -v data_list=`pwd`/$data/wavs/$name \
        -v logdir=$logdir -v name=$name '{
        during=$4-$3
        cmd=$2 sox " - " data_list "/" $1 ".wav" " trim " $3 " " during;
        system(cmd)
        printf("%s %s/%s.wav\n", $1, data_list, $1);
        }' | \
       sort > ${data_list}/wavs_${name}.scp || exit 1;
} &
done
wait
cat ${data_list}/wavs_*.scp > $outscp
rm ${data_list}/wavs_*.scp
