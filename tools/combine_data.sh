...
# This script combines the data_list from multiple source directories into
...
extra_files= # specify additional files in 'src-data_list-dir' to merge, ex. "file1 file2 ..."
...
if $has_segments; then
  for in_dir in $*; do
    if [ ! -f $in_dir/segments ]; then
      ...
      utils/data_list/get_segments_for_data.sh $in_dir
    else
      cat $in_dir/segments
    fi
